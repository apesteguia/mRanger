...
class View:
    ...
    def refresh_windows(self):
        ...
        self.right_win = self.main_win.subwin(
            window_height, window_width, 1, window_width
        )
    ...
    def dirs_menu(self, dirs):
            ...
            if i < len(dirs):
                if self.selected == i:
                    self.left_win.addstr(
                        i - start_index + 1, 1, dirs[i], curses.color_pair(3)
                    )
                else:
        ...
        if self.selected >= height - 1:
            self.left_win.addstr(
                height - 1, 1, dirs[self.selected], curses.color_pair(3)
            )
